...
public class PutFileSocketClient extends Thread {
	...
	public PutFileSocketClient(String host, int port,
			FileCopyParameters params, int threads) {
		this.host = host;
		this.port = port;
		this.params = params;
	...
	public static void main(String[] arg) throws IOException,
		...
		PutFileSocketClient client = new PutFileSocketClient(host, port,
				fileCopyParameters, threads);
		client.process();
	}
	...
	private void process() throws UnknownHostException, IOException,
		...
		if (fileDetailList.size() > 0) {
			...
			PutFilesMergeHelper mergeHelper = new PutFilesMergeHelper(host, port,splitFileDetailsList,params);
			mergeHelper.process();
			
			logger.info("Files Merged Successfully");
			System.out.println("Files Merged Successfully");
	
			PutFilesValidateHelper validationHelper = new PutFilesValidateHelper(host, port,splitFileDetailsList,params);
